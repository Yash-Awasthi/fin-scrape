...
def cmd_show(args):
    ...
    for q in exp.questions:
        ...
        print(
            f"       Type: {q.question_type} | Responses: {len(q.responses)} | Rankings: {len(q.rankings)}"
        )
        if q.ground_truth:
...
def cmd_add_question(args):
    ...
    q = Question(text=args.text, question_type=QuestionType(args.type), ground_truth=args.truth)
...
def cmd_add_ranking(args):
    ...
    ranking = Ranking(judge=args.judge, rankings=rankings, confidence=args.confidence)
...
def main():
    parser = argparse.ArgumentParser(
        description="LLM Deliberate CLI", formatter_class=argparse.RawDescriptionHelpFormatter
    )
