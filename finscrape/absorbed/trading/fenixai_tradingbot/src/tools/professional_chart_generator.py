...
def calculate_supertrend(
    ...
    for i in range(period, n):
        if (
            np.isnan(upper_band[i - 1])
            or np.isnan(lower_band[i - 1])
            or np.isnan(upper_band[i])
...
def calculate_pivot_points(
    ...
    """Calcula classic pivot points usando la vela más reciente."""
    pp = (high[-1] + low[-1] + close[-1]) / 3
    r1, s1 = 2 * pp - low[-1], 2 * pp - high[-1]
    hl_range = high[-1] - low[-1]
    r2, s2 = pp + hl_range, pp - hl_range
...
class ProfessionalChartGenerator:
    ...
    def __init__(
        self, save_path: str = "cache/charts", width: int = 1280, height: int = 720, dpi: int = 150
    ):
    ...
    def archive_old_charts(self, max_age_days: int = 7) -> int:
        """Compress and move old charts to an archive subfolder for future training.

        Charts are NOT deleted — they are gzipped and moved to cache/charts/archive/
        so they remain available as training data for future vision models.
        ...
        archive_dir = self.save_path / "archive"
        archive_dir.mkdir(parents=True, exist_ok=True)
        cutoff = _time.time() - (max_age_days * 86400)
        archived = 0
        for f in self.save_path.glob("*_pro.png"):
            try:
                if f.stat().st_mtime < cutoff:
                    # Compress with gzip and move to archive
                    archive_path = archive_dir / f"{f.name}.gz"
                    with open(f, "rb") as src, gzip.open(archive_path, "wb") as dst:
                        shutil.copyfileobj(src, dst)
                    f.unlink()  # Remove original (compressed copy saved)
                    archived += 1
            except Exception:
                pass
        if archived:
            logger.info(f"Archived {archived} charts to {archive_dir} (compressed)")
        return archived
    ...
    def _prepare_dataframe(self, kline_data: dict[str, list]) -> pd.DataFrame | None:
        ...
        try:
            ...
            if "pivots" in show_indicators:
                ...
                for name, value in [
                    ...
                    color = (
                        self.theme["resistance"]
                        if name.startswith("R")
                        else self.theme["support"] if name.startswith("S") else self.theme["pivot"]
                    )
                    fig.add_hline(
                        y=value,
                        ...
                        annotation_yshift=pivot_label_yshift[name],
    ...
    def _generate_summary(
        ...
        if "ema_50" in show_indicators:
            ema_50 = calculate_ema(close, 50)
            summary["ema_50"] = {
                "value": float(ema_50[-1]),
                "position": "above" if close[-1] > ema_50[-1] else "below",
            }

        if "sma_50" in show_indicators:
            sma_50 = calculate_sma(close, 50)
            summary["sma_50"] = {
                "value": float(sma_50[-1]) if not np.isnan(sma_50[-1]) else None,
                "position": "above" if close[-1] > sma_50[-1] else "below",
            }
        ...
        if "vwap" in show_indicators and volume is not None:
            ...
            summary["vwap"] = {
                "value": float(vwap[-1]),
                "position": "above" if close[-1] > vwap[-1] else "below",
            }

        if "supertrend" in show_indicators:
            st_line, st_dir = calculate_supertrend(high, low, close)
            valid = st_line[~np.isnan(st_line)]
            summary["supertrend"] = {
                "value": float(valid[-1]) if len(valid) else None,
                "direction": "bullish" if st_dir[-1] == 1 else "bearish",
            }

        if "pivots" in show_indicators:
            pivots = calculate_pivot_points(high, low, close)
            summary["pivots"] = {key: float(value) for key, value in pivots.items()}
...
_chart_generator: ProfessionalChartGenerator | None = None
