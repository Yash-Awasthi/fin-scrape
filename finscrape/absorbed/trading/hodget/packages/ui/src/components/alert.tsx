  ...
  "group/alert relative grid w-full gap-0.5 rounded-none border px-2.5 py-2 text-left text-xs has-data-[slot=alert-action]:relative has-data-[slot=alert-action]:pr-18 has-[>svg]:grid-cols-[auto_1fr] has-[>svg]:gap-x-2 *:[svg]:row-span-2 *:[svg]:translate-y-0 *:[svg]:text-current *:[svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        ...
        // Semantic variants — layered on top of the standard set.
        info: "border-transparent bg-blue-100 text-blue-700 *:data-[slot=alert-description]:text-blue-800 *:[svg]:text-blue-600",
        success:
          "border-transparent bg-green-100 text-green-700 *:data-[slot=alert-description]:text-green-800 *:[svg]:text-green-600",
        warning:
          "border-transparent bg-yellow-100 text-yellow-800 *:data-[slot=alert-description]:text-yellow-800 *:[svg]:text-yellow-700",
      },
    },
    defaultVariants: {
      variant: "default",
